...
const createContext = async (skipTlsVerification: boolean = false, headers?: { [key: string]: string }) => {
  ...
  const userAgentFromHeaders = headers?.['user-agent'];
...
app.post('/scrape', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (headers) {
      // Remove user-agent from extra headers since it's now handled in createContext
      const { 'user-agent': _userAgent, ...otherHeaders } = headers;
