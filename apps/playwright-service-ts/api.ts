...
app.post('/scrape', async (req: Request, res: Response) => {
  ...
  // Extract user-agent from headers if present (Playwright ignores user-agent in setExtraHTTPHeaders if already set on context)
  const customUserAgent = headers?.['user-agent'] || headers?.['User-Agent'];
  
  // Filter out user-agent from headers since it's set at context level
